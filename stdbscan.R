...
stdbscan = function (data, eps, eps2, minpts = 7, distance="euclidean", seeds = TRUE, countmode = 1:nrow(data)) {  
    
    data_spasial<- dist(cbind(data$latitude,data$longitude), method=distance)
    data_temporal<- dist(data$date, method=distance)
    ...
    data_temporal <- as.matrix(data_temporal)
      ...
      # data_temporal<- Dist(data$date, method=distance)
      # 
      # noum <- seq(1:dim(data_spasial)[1])
      # noum <- sprintf("%s",noum)
      # colnames(data_spasial) <- noum
      # rownames(data_spasial) <- noum
